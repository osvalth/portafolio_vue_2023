  ...
  <section>
    <div class="inicio">
      <div class="hover-3d-container">
        <div class="hover-3d-element" @mousemove="move3DEffect" @mouseout="reset3DEffect">
      ...
      <div class="info">
        ...
        <li>
          <router-link :to="{ path: '/sobre-mi/mas-sobre-mi'}"><a>Más sobre mí</a></router-link>
        </li>
      </div>
    </div>
...
<script setup>
import MasSobreMi from './MasSobreMi.vue'
</script>
<script>
...
export default {
  ...
  methods: {
    ...
    move3DEffect(event) {
      ...
      element.style.transform = `rotateY(${x * 30}deg) rotateX(${y * 30}deg) scale(1.1)`;
    },
    reset3DEffect() {
      const element = this.$el.querySelector('.hover-3d-element');
...
.hover-3d-container {
  ...
  height: 430px; /* Alto del contenedor */
  perspective: 1000px;
  position: relative;
  /* margin: 50px auto; */
}
...
.hover-3d-element {
  width: 100%;
  height: 100%;
  background-image: url('../assets/img/yo.jpg'); /* Reemplaza 'tu-imagen.jpg' con la URL de tu imagen */
  background-size: cover;
  background-position: center;
  transition: transform 0.05s, box-shadow 0.05s;
  transform-origin: center center; /* Establece el punto de origen de la transformación en el centro */
  transform-style: preserve-3d; /* Para mantener el contenido en 3D */
